...
```bash
novoselz@drum $ mkdir -p ~/workspace/archiving/payload
novoselz@drum $ cd ~/workspace/archiving/payload
...
```bash
novoselz@drum $ cd ..
novoselz@drum $ tar -cvf payload.tar payload/
```

Создание архива только по маске:
```bash
novoselz@drum $ tar -cvf docs_backup.tar payload/*.txt
```
...
```bash
novoselz@drum $ gzip -c payload.tar > payload.tar.gz
novoselz@drum $ bzip2 -c payload.tar > payload.tar.bz2
```

Сравнение веса файлов:
```bash
novoselz@drum $ ls -lh payload.*
# -rw-r--r--  1 novoselz  novoselz   12K May 21 10:20 payload.tar
# -rw-r--r--  1 novoselz  novoselz  1.4K May 21 10:21 payload.tar.bz2
# -rw-r--r--  1 novoselz  novoselz  1.8K May 21 10:21 payload.tar.gz
```

### 2.4. Извлечение данных
Распаковка `tar.gz` архива:
```bash
novoselz@drum $ mkdir unpacked
novoselz@drum $ tar -xzf payload.tar.gz -C unpacked/
```
